        ...
        <div class="card mb-3 text-dark">
            @if ($posts[0]->image)
                <div style="max-height: 500px; overflow: hidden;">
                    <img src="{{ asset('storage/' . $posts[0]->image) }}" class="card-img-top img-fluid rounded mt-3" alt="{{ $posts[0]->category->name }}">
                </div>
            @else
            ...
            <div class="card-body text-center">
                ...
                <p>
                    <small>
                        ...
                        in <a href="/posts?category={{ $posts[0]->category->slug }}">{{ $posts[0]->category->name }}</a> {{ $posts[0]->created_at->diffForHumans() }}
                    </small>
                </p>
            ...
            @foreach ($posts->skip(1) as $post)
                <div class="col-md-4 mb-3">
                    <div class="card border-dark">
                        <div class="position-absolute px-3 py-2 rounded-end" style="background-color: rgba(24,24,27, 0.7)"><a href="/posts?category={{ $post->category->slug }}" class="text-light text-decoration-none">{{ $post->category->name }}</a></div>
